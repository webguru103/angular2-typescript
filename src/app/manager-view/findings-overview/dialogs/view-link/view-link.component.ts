...
export class ViewLinkComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MdDialogRef<ViewLinkComponent>) { }

  ngOnInit() {
    this.filter = new FindingsActionsDataTableFilterModel(this.findingId, this.nodeId, this.nodeType, FindingGroupType.ViewLink);
  ...
  initColumns() {
    this.columns = [
      { name: FindingsDataTableColumns.SerialNumber.name, headerTemplate: this.serialNumberHeaderTemplate, prop: 'serialNumber', sortable: true, width: 200 },
      { name: FindingsDataTableColumns.Severity.name, headerTemplate: this.severityHeaderTemplate, prop: 'severity', sortable: true, width: 100 },
      { name: FindingsDataTableColumns.Category.name, headerTemplate: this.categoryHeaderTemplate, prop: 'type', sortable: true, width: 210 },
      { name: FindingsDataTableColumns.Layer.name, headerTemplate: this.layerHeaderTemplate, prop: 'layer', sortable: true, width: 100 },
      { name: FindingsDataTableColumns.Surface.name, headerTemplate: this.surfaceHeaderTemplate, prop: 'surface', sortable: true, width: 120 },
      { name: FindingsDataTableColumns.R.name, headerTemplate: this.rHeaderTemplate, prop: 'distanceToRoot', sortable: true, width: 100 },
      { name: FindingsDataTableColumns.L.name, headerTemplate: this.lHeaderTemplate, prop: 'lengthMm', sortable: true, width: 100 },
      { name: FindingsDataTableColumns.A.name, headerTemplate: this.aHeaderTemplate, prop: 'areaMm2', sortable: true, width: 100 },
    ];
  }
  ...
  initializeQuickFilters() {
    this.defectGroupActionsService.getQuickFiltersForTimeAndViewLink(this.findingId, this.filter.type, this.filter.id, null, null, FindingGroupType.ViewLink)
      .subscribe(data => {
  ...
  quickFilterSubmitted(columnName: string, quickFilterList: QuickFilterListItemModel[]) {
    ...
    for (const key in this.appliedQuickFiltersColumnNames) {
      if (this.appliedQuickFiltersColumnNames.hasOwnProperty(key)) {
        ...
        this.defectGroupActionsService.getQuickFiltersForTimeAndViewLink(this.findingId, this.filter.type, this.filter.id, checkedQuickFilters, null, FindingGroupType.ViewLink)
          .subscribe(data => {
    ...
    this.defectGroupActionsService.getQuickFiltersForTimeAndViewLink(this.findingId, this.filter.type, this.filter.id, checkedQuickFiltersModel, null, FindingGroupType.ViewLink)
      .subscribe(data => {
  ...
  link() {
    this.defectGroupActionsService.link(this.findingId, this.selected[0].id, FindingGroupType.ViewLink).subscribe(() => {
